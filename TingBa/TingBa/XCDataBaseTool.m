...
+(FMDatabaseQueue *)getFMDataBase{
    ...
    if (!queue) {
        ...
        queue=[FMDatabaseQueue databaseQueueWithPath:[XCDataBaseTool getFilePath]];
        //创建表
        [XCDataBaseTool createTable];
    }
...
+(void)createTable{
    ...
    [[XCDataBaseTool getFMDataBase]inDatabase:^(FMDatabase *db) {
     //创建表 执行多条sql语句
